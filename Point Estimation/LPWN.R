...
#'@keywords internal
Bmat<-function(d,m,T,R_short,R_noise){
...
LPWN<-function(data, m, R_short=0, R_noise=0){
  ...
  est<-optim(par=param.vec, fn=R.LPWN, m=m, data=data, R_short=R_short, R_noise=R_noise, method="L-BFGS-B", lower=c(0.01,rep(-50,R_short),10^(-6),rep(-10,R_noise)), upper=c(0.99,rep(50,R_short),10^6,rep(10,R_noise)))$par
  ...
  B_mat<-Bmat(d,m,T,R_short,R_noise) # variance stabilizing transformation
...
LPWN_general <- function(
  ...
  gamma_m <- tuning_const / log(m_val)
  ...
  # Case 2: need differencing
  ijk <- 1

  while (d_est >= (1 - gamma_m)) {

    data_diff <- diff(data, differences = ijk)
